...
const daysToDeadlineByPercent = computed(() => {
   ...
   return Math.ceil((_maxBorrowDays - daysToDeadline) / _maxBorrowDays * 100)
})
</script>
         ...
         <div class="flex flex-col gap-2 mt-2">
            ...
            <div :class="['flex justify-end flex-col col-span-2']">
               ...
               >
                  ...
                  <div
                     v-if="isShowThumb"
                     :class="['z-10 absolute left-0 top-0 h-full', statusData[status].bg]"
                     :style="{
                        width: `${Math.max(0, Math.min(100, daysToDeadlineByPercent))}%`,
                     }"
                  ></div>
               </div>
            </div>
